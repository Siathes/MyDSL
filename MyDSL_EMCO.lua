-- =============================================================================
-- MyDSL_EMCO.lua -- ported EMCO (Embeddable Multi Console Object) engine
-- =============================================================================
-- Cannibalized from EMCO 2.9.0 (Damian "demonnic" Monogue, MIT license,
-- https://github.com/demonnic/EMCO), per Steven ("fold emco into mydsl...
-- clean rip... cannibalize emco like we did pnp"), 2026-07-17 -- confirmed
-- a real blocker: the fresh-install package built the same day depended on
-- the separate EMCOChat package being installed too, and it wasn't.
--
-- Source: ~/Downloads/EMCO-2.9.0.zip, src/resources/emco.lua (2,353 lines).
-- Ported essentially verbatim -- this is the actual tested class, not a
-- reimplementation -- with exactly two changes, both marked inline where
-- they occur:
--   1. The loggingconsole.lua require (an optional feature MyDSL never
--      configures) used (...):match(...) to detect its own dotted
--      require() path -- that assumption breaks under dofile() (no such
--      vararg context exists), so it's replaced with a plain `local LC =
--      nil`, which is exactly the state every downstream nil-guarded LC
--      reference already falls back to safely.
--   2. Exposed as MyDSL.EMCO at the end (in addition to the original
--      `return EMCO`), matching this project's own MyDSL.X convention
--      instead of relying on capturing dofile()'s return value.
-- demontools.lua and loggingconsole.lua (EMCO's sibling files in the same
-- repo) were NOT ported -- confirmed via grep that emco.lua itself never
-- references demontools at all (it's a separate, unrelated utility library
-- Damian bundles in the same repo), and loggingconsole is the optional,
-- unused-by-MyDSL feature covered by change #1 above.
--
-- EMCO's own self-update alias ("emco update", which uninstallPackage()s
-- and reinstalls from GitHub) was never part of this file -- it lives in
-- EMCOChat's separate native alias tree, not in emco.lua itself, so it
-- simply doesn't exist to port. MyDSL_ChatWrapper.lua's existing
-- disableEmcoUpdateAlias() safety call is left in place as a harmless
-- no-op if that package is ever also present for some other reason.
-- =============================================================================
