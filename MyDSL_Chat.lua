...
-- inherited from the separate EMCOChat package this project no longer
-- depends on) alongside a MyDSL-side mirror at C.emco; now C.emco
-- (== MyDSL.Chat.emco) is the only copy. MyDSL_ChatTriggers.lua and
-- MyDSL_DataLayer.lua's one reference were both updated to match.
--
-- ---- PART 1: the ported EMCO class ----------------------------------
-- Cannibalized from EMCO 2.9.0 (Damian "demonnic" Monogue, MIT license,
-- https://github.com/demonnic/EMCO), src/resources/emco.lua (2,353
-- lines), ported essentially verbatim -- see the class body below for
-- the two real changes (both marked inline where they occur): the
-- loggingconsole.lua optional-require replaced with a plain `local LC =
-- nil` (unused by MyDSL, and its own require-path self-detection
-- doesn't survive dofile() instead of require()); and exposed as
-- MyDSL.EMCO in addition to the original `return EMCO`. demontools.lua
-- (EMCO's sibling file in the same repo) was not ported -- confirmed via
-- grep emco.lua itself never references it at all.
--
-- ---- PART 2: chat window management (was MyDSL_ChatWrapper.lua) -----
-- Creates/configures the MyDSL-owned chat tab console using the proven
-- Emberline creation model:
--
--   C.emco = MyDSL.EMCO:new(config, MyDSL_Chat_UserWindow)
--
-- Design:
--   - Creates/recreates C.emco inside MyDSL_Chat UserWindow.
--   - Keeps normal EMCO append API -- MyDSL_ChatTriggers.lua calls
--     MyDSL.Chat.emco:append(tabName).
--   - Does NOT install chat capture triggers (MyDSL_ChatTriggers.lua's job).
--   - Does NOT route channels (also MyDSL_ChatTriggers.lua).
--   - Does NOT use Geyser.changeContainer.
--   - Does NOT dock/move the old native EMCOPrebuiltChatContainer (hidden
--     forever by hideOldPrebuilt() -- this project owns the whole chat
--     stack now, that container is dead weight from the pre-cannibalized
--     era, kept hidden rather than referenced).
--   - Does NOT use EMCO mapTab.
--   - Uses tabs: All, Local, City, OOC, Tells, Group
-- =============================================================================
...
-- loggingconsole.lua deliberately not ported -- unused by MyDSL (no
-- config option here ever sets a log path), and every LC reference below
-- is already nil-guarded in the real source (confirmed by reading all 6
-- call sites before cutting this), so this falls through to the same
-- plain Geyser.MiniConsole every other MyDSL window already uses.
local LC = nil
...
MyDSL = MyDSL or {}
MyDSL.EMCO = EMCO
-- No "return EMCO" here -- valid as the last statement of EMCO's own
-- standalone chunk (which this class body originally was, and Lua
-- requires `return` to be the final statement of a chunk), but this is
-- no longer the end of the file now that ChatWrapper's code follows
-- immediately below in the same merged module. MyDSL.EMCO above is the
-- only exposure needed.
...
function C.createInWindow()
  ...
  if MyDSL and MyDSL.Alpha and MyDSL.Alpha.verbose then ce("created MyDSL.Chat.emco inside MyDSL_Chat with tabs: " .. tabListString()) end
  return true
end
...
function C.clear()
  local ch = C.emco
  if not ch then ce("no chat instance to clear"); return end
...
function C.echoTest(tab, msg)
  ...
  local ch = C.emco
  if not ch then ce("no chat instance"); return end
